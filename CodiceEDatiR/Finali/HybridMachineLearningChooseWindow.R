...
main_advanced_forecasting <- function() {
  ...
  # 2. CONFIGURAZIONE TEST WINDOW INTERATTIVA
  # ==============================================================================
  
  cat("⚙️ ===== CONFIGURAZIONE FINESTRA DI TEST ===== ⚙️\n")
  
  # Mostra periodo disponibile
  data_inizio <- min(serie_mensile$data_mese)
  data_fine <- max(serie_mensile$data_mese)
  
  cat("📊 Dati disponibili dal", format(data_inizio, "%Y-%m"), 
      "al", format(data_fine, "%Y-%m"), "\n")
  cat("📏 Totale:", nrow(serie_mensile), "mesi di dati\n\n")
  
  # INPUT 1: Anno e mese di inizio test
  repeat {
    cat("🎯 Da quale data vuoi iniziare il TEST SET?\n")
    
    # Input anno
    repeat {
      anno_test <- as.numeric(readline(prompt = paste0("   Anno (da ", year(data_inizio), " a ", year(data_fine), "): ")))
      if(!is.na(anno_test) && anno_test >= year(data_inizio) && anno_test <= year(data_fine)) {
        break
      }
      cat("   ❌ Anno non valido. Inserisci un anno tra", year(data_inizio), "e", year(data_fine), "\n")
    }
    
    # Input mese
    repeat {
      mese_test <- as.numeric(readline(prompt = "   Mese (1-12): "))
      if(!is.na(mese_test) && mese_test >= 1 && mese_test <= 12) {
        break
      }
      cat("   ❌ Mese non valido. Inserisci un numero tra 1 e 12\n")
    }
    
    # Crea data di cutoff
    data_cutoff_test <- as.Date(paste(anno_test, sprintf("%02d", mese_test), "01", sep = "-"))
    
    # Verifica che la data sia valida
    if(data_cutoff_test >= data_inizio && data_cutoff_test <= data_fine) {
      # Verifica che ci siano abbastanza dati per training (almeno 24 mesi)
      mesi_training <- sum(serie_mensile$data_mese < data_cutoff_test)
      
      if(mesi_training >= 24) {
        break
      } else {
        cat("   ❌ Dati insufficienti per training. Servono almeno 24 mesi prima del", 
            format(data_cutoff_test, "%Y-%m"), "\n")
        cat("      Con questa data avresti solo", mesi_training, "mesi di training\n")
        cat("      Prova una data più tarda\n\n")
      }
    } else {
      cat("   ❌ Data non valida. Deve essere tra", format(data_inizio, "%Y-%m"), 
          "e", format(data_fine, "%Y-%m"), "\n\n")
    }
  }
  
  # Calcola mesi disponibili per test
  mesi_disponibili_test <- sum(serie_mensile$data_mese >= data_cutoff_test)
  
  cat("✅ Data test selezionata:", format(data_cutoff_test, "%Y-%m"), "\n")
  cat("📊 Mesi disponibili per training:", sum(serie_mensile$data_mese < data_cutoff_test), "\n")
  cat("📊 Mesi disponibili per test:", mesi_disponibili_test, "\n\n")
  
  # INPUT 2: Numero mesi da prevedere
  repeat {
    cat("🔮 Quanti mesi vuoi PREVEDERE?\n")
    cat("   💡 Puoi prevedere da 1 a", mesi_disponibili_test, "mesi\n")
    cat("   💡 Suggerimento: usa parte dei mesi per validazione e parte per previsioni future\n")
    
    h_forecast <- as.numeric(readline(prompt = paste0("   Numero mesi (1-", mesi_disponibili_test, "): ")))
    
    if(!is.na(h_forecast) && h_forecast >= 1 && h_forecast <= mesi_disponibili_test) {
      break
    }
    cat("   ❌ Numero non valido. Inserisci un numero tra 1 e", mesi_disponibili_test, "\n\n")
  }
  
  cat("✅ Orizzonte previsioni:", h_forecast, "mesi\n\n")
  
  # ==============================================================================
  # 3. SPLIT DATI BASATO SU CONFIGURAZIONE UTENTE
  # ==============================================================================
  
  train_data <- serie_mensile %>% filter(data_mese < data_cutoff_test)
  test_data <- serie_mensile %>% filter(data_mese >= data_cutoff_test)
  
  # Limita test_data al numero di mesi richiesti
  if(nrow(test_data) > h_forecast) {
    test_data <- test_data[1:h_forecast, ]
  }
  
  cat("📊 CONFIGURAZIONE FINALE:\n")
  cat("   • Training: da", format(min(train_data$data_mese), "%Y-%m"), 
      "a", format(max(train_data$data_mese), "%Y-%m"), 
      "(", nrow(train_data), "mesi )\n")
  cat("   • Test/Previsioni: da", format(min(test_data$data_mese), "%Y-%m"), 
      "a", format(max(test_data$data_mese), "%Y-%m"), 
      "(", nrow(test_data), "mesi )\n")
  cat("   • Orizzonte previsioni:", h_forecast, "mesi\n\n")
  
  # Conferma dall'utente
  repeat {
    conferma <- toupper(readline(prompt = "Procedere con questa configurazione? (S/N): "))
    if(conferma %in% c("S", "SI", "Y", "YES")) {
      break
    } else if(conferma %in% c("N", "NO")) {
      cat("❌ Operazione annullata. Riavvia la funzione per una nuova configurazione.\n")
      return(NULL)
    } else {
  ...
  cat("\n🚀 Avvio analisi con configurazione utente...\n\n")
  ...
  return(list(model = final_model, 
              ...
              configuration = list(
                cutoff_date = data_cutoff_test,
                forecast_horizon = h_forecast,
                train_months = nrow(train_data),
                test_months = nrow(test_data)
              )))
}
